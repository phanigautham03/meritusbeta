...
function Login() {
  ...
  return (
    <AuthSplit
      title="Welcome back"
      subtitle="Your rank is waiting."
      bullets={["Pick up your study streak", "See your Forget-Meter alerts", "Continue your AI study plan"]}
    >
      <h2 className="text-2xl font-bold text-body">Log in to Meritus</h2>
      <p className="text-sm text-secondary-text mt-1">Good to see you again.</p>

      <form onSubmit={(e) => { e.preventDefault(); nav({ to: "/dashboard" }); }} className="mt-6 space-y-4">
        <div>
          <Label htmlFor="email">Email</Label>
          <Input id="email" type="email" placeholder="you@example.com" className="mt-1.5 h-11" required />
        </div>
        <div>
          ...
          <Input id="password" type="password" className="mt-1.5 h-11" required />
        </div>
        <Button type="submit" className="w-full h-11 font-semibold">Log in</Button>
      </form>
      ...
      <p className="text-sm text-secondary-text text-center mt-5">
        New here? <Link to="/signup" className="text-primary font-semibold hover:underline">Sign up</Link>
      </p>
    </AuthSplit>
  );
}
